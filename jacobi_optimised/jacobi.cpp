...
bfloat16* golden_jacobi(bfloat16 * domain_data, int x, int y, int num_its) {
    ...
    printf("Calculated reference version over %d iterations in %.4f secs\n", num_its, elapsed_time);
    delete [] ukp1;
    return uk;
}
...
CoreRangeSet constructCoreRanges(int num_in_x, int num_in_y, int offset_x) {
    std::set<CoreRange> ranges;
    for (int i=0;i<num_in_x;i++) {
        for (int j=0;j<num_in_y;j++) {
            CoreCoord core = {i+offset_x, j};
...
Program generateProgram(int cores_in_x, int cores_in_y, int offset_x, const vector<uint32_t> compute_kernel_args, vector<uint32_t> stream_in_runtime_args, vector<uint32_t> stream_out_runtime_args) {
    Program program = CreateProgram();

    CoreRangeSet all_cores=constructCoreRanges(cores_in_x, cores_in_y, offset_x);
    ...
    CBHandle cb_0_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_input0_config);
    ...
    CBHandle cb_1_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_input1_config);
    ...
    CBHandle cb_2_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_input2_config);
    ...
    CBHandle cb_3_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_input3_config);
    ...
    CBHandle c_intermed0_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_intermediate1_config);
    ...
    CBHandle c_intermed1_src = tt_metal::CreateCircularBuffer(program, all_cores, cb_intermediate2_config);

    uint32_t semaphore_id = tt_metal::CreateSemaphore(program, all_cores, static_cast<uint32_t>(0));

    stream_in_runtime_args.push_back(static_cast<uint32_t>(semaphore_id));
    stream_out_runtime_args.push_back(static_cast<uint32_t>(semaphore_id));

    for (int ci=0;ci<cores_in_x;ci++) {
        for (int cj=0;cj<cores_in_y;cj++) {
            vector<uint32_t> specialised_compute_kernel_args;
            specialised_compute_kernel_args.assign(compute_kernel_args.begin(), compute_kernel_args.end());  
            specialised_compute_kernel_args.push_back(static_cast<uint32_t>(ci+offset_x));
            specialised_compute_kernel_args.push_back(static_cast<uint32_t>(cj));

            CoreCoord core = {ci+offset_x, cj};
            ...
            KernelHandle compute_kernel = CreateKernel(
                ...
                ComputeConfig{
                    ...
                    .compile_args = specialised_compute_kernel_args,
                }
            );
        }
    }
    ...
    SetRuntimeArgs(program, stream_in_kernel, all_cores, stream_in_runtime_args);
    SetRuntimeArgs(program, stream_out_kernel, all_cores, stream_out_runtime_args);
    return program;
}
...
int main(int argc, char **argv) {
    ...
    if (argc !=6) {
        fprintf(stderr, "Must provide five arguments, the problem size in x, problem size in y and number of iterations, cores in x and y\n");
        return -1;
    }
    ...
    int my_cores_x=atoi(argv[4]);
    int my_cores_y=atoi(argv[5]);
    ...
    printf("Executing over x=%d, y=%d cores, there are a total of x=%d y=%d cores available\n", my_cores_x, my_cores_y, num_cores_x, num_cores_y);
    ...
    const vector<uint32_t> compute_kernel_args = {
        ...
        static_cast<uint32_t>(my_cores_x),
        static_cast<uint32_t>(my_cores_y),
    };

    std::vector<uint32_t> stream_in_runtime_args = {
    ...
    std::vector<uint32_t> stream_out_runtime_args = {
        domain_data_dram_buffer->address(),
        result_data_dram_buffer->address(),
        static_cast<uint32_t>(x_size),
        static_cast<uint32_t>(y_size),
    ...
    double exec_time;
    if (my_cores_x * my_cores_y > 50) {
        Program program1=generateProgram(my_cores_x/2, my_cores_y, 0, compute_kernel_args, stream_in_runtime_args, stream_out_runtime_args);
        Program program2=generateProgram(my_cores_x/2, my_cores_y, my_cores_x/2, compute_kernel_args, stream_in_runtime_args, stream_out_runtime_args);
        gettimeofday(&start_time, NULL);
        ...
        Program program=generateProgram(my_cores_x, my_cores_y, 0, compute_kernel_args, stream_in_runtime_args, stream_out_runtime_args);
        gettimeofday(&start_time, NULL); 
    ...
    long total_points=x_size * y_size * num_its;
