...
void kernel_main() {
    ...
    std::uint32_t x_size  = get_arg_val<uint32_t>(2);
    std::uint32_t y_size  = get_arg_val<uint32_t>(3);
    std::uint32_t num_its  = get_arg_val<uint32_t>(4);
    uint32_t page_size = get_arg_val<uint32_t>(5);
    uint32_t cores_in_x = get_arg_val<uint32_t>(6);
    uint32_t cores_in_y = get_arg_val<uint32_t>(7);
    uint32_t semaphore_addr = get_arg_val<uint32_t>(8);
        ...
        for (uint32_t bx=my_start_batch_x;bx<my_end_batch_x;bx++) {
            ...
            for (uint32_t by=my_start_batch_y;by<my_end_batch_y;by++) {
                ...
                for (uint32_t i=0;i<BATCH_SIZE_X;i++) {
                    ...
                    std::uint32_t page_id=dram_addr_offset / page_size;
                    std::uint32_t page_offset=dram_addr_offset % page_size;

                    uint64_t domain_data_buffer_noc_addr;
                    if (it % 2 == 0) {
                        domain_data_buffer_noc_addr = addr_gen_two.get_noc_addr(page_id, page_offset);
                    } else {
                        domain_data_buffer_noc_addr = addr_gen_one.get_noc_addr(page_id, page_offset);
                    }
