...
CoreRangeSet constructCoreRanges(int num_in_x, int num_in_y) {
...
int main(int argc, char **argv) {
    ...
    CoreRangeSet all_cores=constructCoreRanges(my_cores_x, my_cores_y);
    ...
    for (int ci=0;ci<my_cores_x;ci++) {
        for (int cj=0;cj<my_cores_y;cj++) {
            CoreCoord core = {ci, cj};
            ...
            vector<uint32_t> data_kernel_args = {
                static_cast<uint32_t>(my_cores_x),
                static_cast<uint32_t>(my_cores_y),
                static_cast<uint32_t>(ci),
                static_cast<uint32_t>(cj),
